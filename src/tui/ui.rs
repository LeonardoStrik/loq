...
pub fn draw_ui(frame: &mut Frame, app: &App) {
    ...
    let editor_block = Block::default().borders(Borders::all()).style(editor_style);

    let console_block = Block::default()
        .borders(Borders::all())
        .style(console_style);
    let env_block = Block::default().borders(Borders::all()).style(env_style);
    let header = Paragraph::new(Text::styled("Loq.rs", header_style))
        .block(header_block)
        .centered();
    let editor = Paragraph::new(Text::styled(FILLER_TEXT, editor_style)).block(editor_block);
    let console = Paragraph::new(Text::styled(FILLER_TEXT, console_style)).block(console_block);
    let env = Paragraph::new(Text::styled(FILLER_TEXT, env_style)).block(env_block);
    let footer = Paragraph::new(Text::styled("Footer", footer_style))
        .block(footer_block)
        .centered();
    frame.render_widget(header, header_area);
    frame.render_widget(editor, editor_area);
    frame.render_widget(console, console_area);
